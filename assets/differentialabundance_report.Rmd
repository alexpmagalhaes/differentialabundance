    ...
    for (pvt in names(p_value_types)){
        ...
        if (params$differential_method == "deseq2") {
            for (assay_type in rev(names(assay_data))){
                ...
                top_genes <- head(contrast_de[order(contrast_de$Padj), 1], 10)
                ...
                plot_df$Condition <- obs_sub[plot_df$Sample, "treatment"]
                gene_names <- contrast_de$`Gene name`
                names(gene_names) <- contrast_de$`Gene id`  # named vector for lookup
                plot_df$GeneName <- gene_names[plot_df$Gene]

                pvals <- contrast_de$Padj
                names(pvals) <- contrast_de$`Gene id`
                plot_df$pval <- pvals[plot_df$Gene]
