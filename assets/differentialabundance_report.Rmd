...
for (i in 1:nrow(contrasts)){
    ...
    for (pvt in names(p_value_types)){
        ...
        for (assay_type in rev(names(assay_data))){
            ...
            # library(reshape2)
            plot_df <- reshape2::melt(expr_mat[top_genes, ])

            features <- prettifyVariablename(params$features_type)

            colnames(plot_df) <- c(features, "Sample", "Expression")
            ...
            plot_df$GeneName <- gene_names[plot_df[, features]]

            pvals <- contrast_de[,qval_column]
            names(pvals) <- contrast_de$features_id_col
            plot_df$pval <- pvals[plot_df[,features]]

            # # Group p-values to add to plot
            pvals_grouped <- plot_df %>%
                group_by(GeneName) %>%       # replace with your facetting/grouping var
