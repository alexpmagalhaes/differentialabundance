    ...
    for (pvt in names(p_value_types)) {
        ...
        if (length(parts) > 0) {
            ...
            for (assay_type in rev(names(assay_data))){
                ...
                # Get gene names and add them as a new "GeneName" column to plot_df
                label_col <- prettifyVariablename(label_col)
                gene_names <- combined[, label_col]
                features_id_col <- prettifyVariablename(params$features_id_col)
                names(gene_names) <- combined[, features_id_col]  # named vector for lookup
                plot_df$GeneName <- gene_names[plot_df[[1]]]
                ...
                p <- ggplot(plot_df, aes(x = Condition, y = Expression)) +
                        ...
                        ggbeeswarm::geom_quasirandom(width = 0.2, aes(color = Condition)) +
                        facet_wrap(~ GeneName, scales = "free_y") +
                        # scale_fill_manual(values = groupColorScale) +
                        ...
                        geom_text(data = pvals_grouped,
                            aes(x = 1.75, y = max(plot_df$Expression) * 1.01,  # position above boxes
