...
names(differential_results) <- paste0(contrasts$id, '_', params$study_name)
```
...
if (!is.null(params$functional_method)){
    ...
    if (params$functional_method == 'gsea') {
        ...
        reference_gsea_tables <- paste0(contrasts$id, '_', params$study_name, ".", gmt_name, '.gsea_report_for_', contrasts$reference, '.tsv')
        target_gsea_tables <- paste0(contrasts$id, '_', params$study_name, ".", gmt_name, '.gsea_report_for_', contrasts$target, '.tsv')
        for (i in 1:nrow(contrasts)){
        ...
        for (id in contrasts$id) {
            name <- paste0(id, '_', params$study_name)
            cat(paste0("\n##### ", name, "\n"))
            ...
            if (!file.exists(table_path) || file.size(table_path) == 0){
                ...
                print(htmltools::tagList(datatable(all_enriched, caption = paste('Enriched pathways in', id, " (check", table, "for more detail)"), rownames = FALSE)))
            }
            cat("\n")
        }
    }
