...
for (i in 1:nrow(contrasts)){
    ...
    for (pvt in names(p_value_types)){
        ...
        for (assay_type in rev(names(assay_data))){
            ...
            top_genes <- head(contrast_de[order(contrast_de[,qval_column]), 1], 10)
            ...
            features_name_col <- params$features_name_col

            gene_names <- contrast_de[, 2]

            features_id_col <- params$features_id_col

            names(gene_names) <- contrast_de[, 1]  # named vector for lookup
            plot_df$GeneName <- gene_names[plot_df[[1]]]

            pvals <- contrast_de[,qval_column]
            names(pvals) <- contrast_de[, 1]
            plot_df$pval <- pvals[plot_df[[1]]]

            # Group p-values to add to plot
            pvals_grouped <- plot_df %>%
                group_by(GeneName) %>%
                summarise(pval = unique(pval))
            write.csv(top_genes, "contrast.csv", row.names = FALSE)
            write.csv(plot_df, "plot.csv", row.names = FALSE)
