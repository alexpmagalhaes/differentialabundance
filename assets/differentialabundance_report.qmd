...
params:
    ...
    paramsheet: NULL
    gtf: NULL
    filtering_min_proportion_not_na: NULL
    filtering_min_samples_not_na: NULL
    deseq2_seed: NULL
    limma_use_voom: NULL
    immunedeconv_run: NULL
    immunedeconv_method: NULL
    immunedeconv_function: NULL
    shinyngs_build_app: NULL
    skip_reports: NULL
    shinyngs_deploy_to_shinyapps_io: NULL
...
title:  "<img src=\"`{r} file.path(params$input_dir, params$logo)`\" style=\"float: left;\"/>`{r} I(report_title)`"
subtitle: "`{r} I(report_subtitle)`"
date: "`{r} Sys.Date()`"
--- -->
...
Comparisons were made between `{r} params$observations_type` groups defined using
`{r} params$observations_type` metadata columns, as described in the following table of contrasts:
...
if (!is.null(params$functional_method)){
    cat("\n### Gene set analysis\n")
    cat("\n#### ", toupper(params$functional_method) ,"\n", "\n")
    cat("::: {.panel-tabset}\n")
