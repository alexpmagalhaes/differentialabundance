...
---
title:  "<img src=\"`{r} file.path(params$input_dir, params$logo)`\" style=\"float: left;\"/>`{r} params$report_title`"
...
This report summarises differential `{r} I(params$features_type)` analysis as performed by the nf-core/differentialabundance pipeline.
...
A summary of `{r} I(params$observations_type)` metadata is below:
...
Comparisons were made between `{r} I(params$observations_type)` groups defined using `r params$observation_type` metadata columns, as described in the following table of contrasts:
